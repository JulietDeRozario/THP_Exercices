...
def make_project_folder
  ...
  def check_if_user_gave_input
    abort('[project_generator] Missing parameter project_name. Retry using command `project_generator project_name`') if ARGV.empty?
  end
  ...
  def make_app_file(project_name)
    ...
    # Make prefilled project.rb file
    def make_project_file(project_name)
      file = File.open("#{project_name}/lib/project.rb", 'w')
      file.puts "require 'bundler'"
      file.puts "Bundler.require"
      file.close
      path = system("pwd")
  ...
  def perform
    ...
    # Make prefilled project.rb file
    make_project_file(project_name)
